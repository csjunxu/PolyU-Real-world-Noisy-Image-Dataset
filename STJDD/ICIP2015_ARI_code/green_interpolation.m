function green = green_interpolation(mosaic, mask, pattern, eps)
...
% initial linear interpolation (Eq.(1))
Kh = [1/2,0,1/2];
Kv = Kh';
...
for ittime = 1:itnum
    % generate horizontal and vertical tentative estimate by RI (Eq.(2))
    ...
    % calculate residuals of RI and MLRI (Eq.(4))
    ...
    % horizontal and vertical linear interpolation of residuals (Eq.(5))
    Kh = [1/2,1,1/2];
    ...
    % calcualte iteration criteria (Eq.(7))
