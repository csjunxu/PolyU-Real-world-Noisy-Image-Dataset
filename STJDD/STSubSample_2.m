...
for method = ['A' 'B']
    ...
    if strcmp(method, 'A')==1
        STsubRaw = zeros(2*hm, 2*wm, chm, 'double');
        for i = 1:sam_num
        ...
        imwrite(uint16(STmeanRaw), ['C:/Users/csjunxu/Desktop/TIP2017/RID_Dataset/STA_Results/STA_' D{end} '_' method '.png']);
    elseif strcmp(method, 'B') ==1
        % each WxW patch can be transformed to a R/G/B pixel
        STsubRaw = zeros(hm, wm, chm, 'double');
        for i = 1:sam_num
            S = regexp(im_dir(i).name, '\.', 'split');
            Raw = double(imread([Original_image_dir '/' S{1} '.tiff']));
